...
class JobToInvDelegate {
  ...
  getJobDetail(String partNum, String jbNum) async {
    HttpOverrides.global = MyHttpOverrides();
    String apiUrl = await sharedPref.getString("userUrl");
    String userId = await sharedPref.getString("userName");
    String password = await sharedPref.getString("userPass");
    ...
    Uri url = Uri.parse(
        "$apiUrl/BaqSvc/VSAPP_jobtoinvDetails($company)?Jbnum=$jbNum&PartNum=$partNum&Plant=$plant");
    String basicAuth =
        'Basic ${base64Encode(utf8.encode('$userId:$password'))}';
    Map<String, String> requestHeaders = {
      'Content-type': 'application/json',
  ...
  getDocType() async {
    ...
    Uri url =
        Uri.parse("$apiUrl/BaqSvc/VSApp_TranDocType?pr_TrandocType=WIPStock");
    String basicAuth =
        'Basic ${base64Encode(utf8.encode('$userId:$password'))}';
    Map<String, String> requestHeaders = {
      'Content-type': 'application/json',
  ...
  getPartAsync(String jobNum) async {
    HttpOverrides.global = MyHttpOverrides();
    String apiUrl = await sharedPref.getString("userUrl");
    String userId = await sharedPref.getString("userName");
    String password = await sharedPref.getString("userPass");
    ...
    Uri url =
        Uri.parse("$apiUrl/BaqSvc/VSApp_SerialNumJoToInv/?JobNum=$jobNum");
    String basicAuth =
        'Basic ${base64Encode(utf8.encode('$userId:$password'))}';
    Map<String, String> requestHeaders = {
      'Content-type': 'application/json',
