...
class InspInvDelegate {
  getInspectionList() async {
    ...
    Uri url = Uri.parse(
        "$apiUrl/BaqSvc/VSAPP_InspectionInventoryCards/?pr_Company=$company&pr_Plant=$plant");
    String basicAuth =
  ...
  getInspectionDtls(String nonconfid) async {
    HttpOverrides.global = MyHttpOverrides();
    String apiUrl = await sharedPref.getString("userUrl");
    String userId = await sharedPref.getString("userName");
    String password = await sharedPref.getString("userPass");
    ...
    Uri url = Uri.parse(
        "$apiUrl/BaqSvc/VSAPP_InspectionInventoryDtl/?pr_Company=$company&pr_Plant=$plant&pr_NonConf=$nonconfid");
    String basicAuth =
        'Basic ${base64Encode(utf8.encode('$userId:$password'))}';
    Map<String, String> requestHeaders = {
      'Content-type': 'application/json',
  ...
  getInspectors(String nonconfid) async {
    HttpOverrides.global = MyHttpOverrides();
    String apiUrl = await sharedPref.getString("userUrl");
    String userId = await sharedPref.getString("userName");
    String password = await sharedPref.getString("userPass");
    ...
    Uri url = Uri.parse("$apiUrl/BaqSvc/VSApp_InspectorId($company)");
    String basicAuth =
        'Basic ${base64Encode(utf8.encode('$userId:$password'))}';
    Map<String, String> requestHeaders = {
      'Content-type': 'application/json',
